...
def load_and_prepare_data(
    ...
    Returns:
        A pandas DataFrame with 'id', 'original_id', 'class_name', 'definition', 'embedding_text', and 'default_definition' columns.
    ...
    try:
        ...
        # Ensure all component columns exist, creating them as empty if not.
        # Then, fill any NaN values in these columns with empty strings.
        for col_name in embedding_component_columns:
            if col_name not in df.columns:
                # If a column is entirely missing, create it as an empty series
                # of strings to prevent errors during string concatenation later.
                # pandas (as pd) must be imported for this.
                df[col_name] = pd.Series([""] * len(df), index=df.index, dtype=str)
            else:
...
def create_and_populate_qdrant(
    ...
    Args:
        ...
        data: DataFrame containing 'id', 'original_id', 'class_name', 'definition', 'embedding_text', and 'default_definition'.
    ...
    try:
        if not client.collection_exists(collection_name):
            ...
            # Ensure vector_config.vectors is not None before accessing attributes
            if vector_config.vectors and (
...
if __name__ == "__main__":
    ...
    prepared_data = load_and_prepare_data(
        csv_path="data/unspsc-english-v260801.1.xlsx",  # Your specific CSV
        id_column="Key",  # Column with unique IDs
